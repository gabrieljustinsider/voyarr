...
from security import verify_password, get_password_hash, create_access_token, JWT_SECRET, ALGORITHM
...
def register_user(user: UserCreate, request: Request, db: Session = Depends(get_db)):
    ...
    if user_count > 0:
        ...
        is_authorized = False
        if api_key and api_key == os.getenv("MASTER_KEY"):
            is_authorized = True
        ...
        if not is_authorized:
            raise HTTPException(status_code=403, detail="Only admins or master key holders can register new users.")
    else:
    ...
    db.commit()
    db.refresh(db_user)
    return {"message": "User created successfully", "username": db_user.username, "role": db_user.role}

@router.post("/token", dependencies=[Depends(rate_limit(max_requests=10, window_seconds=60))])
