...
def approve_request(req_id: int, provider_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Admin Only: Approve a request and send it to the download queue."""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Forbidden: Only admins can approve requests")
    ...
    if not db_req.url:
        raise HTTPException(status_code=400, detail="Cannot auto-approve a request without a URL.")
    ...
    py_requests.post(
        ...
        json={"provider_id": provider_id, "url": db_req.url, "action": "metadata_and_download"},
...
def reject_request(req_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Forbidden: Only admins can reject requests")
