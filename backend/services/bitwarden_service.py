...
class BitwardenService:
    ...
    def get_config(db: Session):
        ...
        token_vault = db.query(Vault).filter_by(entity_type="global_setting", key="bw_session_token").first()
    ...
    def push_credentials(db: Session):
        ...
        for cred in creds:
            ...
            try:
                ...
                search_res = requests.get(f"{host}/object/item", headers=headers, timeout=10)
                search_data = search_res.json()
                existing_items = search_data.get('data', []) if isinstance(search_data, dict) else (search_data if isinstance(search_data, list) else [])
                existing_item = next((i for i in existing_items if i.get('name') == f"Voyarr: {provider.name}"), None)
                
                if existing_item:
                    item_id = existing_item.get('id')
                    res = requests.put(f"{host}/object/item/{item_id}", json=item_payload, headers=headers, timeout=10)
                    res.raise_for_status()
                else:
                    res = requests.post(f"{host}/object/item", json=item_payload, headers=headers, timeout=10)
                    res.raise_for_status()
                pushed_count += 1
    ...
    def pull_credentials(db: Session):
        ...
        for item in items:
            try:
                ...
                db.rollback()
                print(f"Failed to pull Bitwarden credential {item.get('name', 'Unknown')}: {e}")
                continue
