...
from models import NotificationPreference, NotificationRule, NotificationLog, UserPreference, User, Favorite
...
class NotificationService:
    ...
    def send_discord_webhook(url: str, message: str):
        ...
        try:
            ...
            if resp.status_code not in [200, 204]:
                logger.error(f"Discord webhook returned status {resp.status_code}: {resp.text}")
        except Exception as e:
    ...
    def send_discord_dm(bot_token: str, discord_user_id: str, message: str):
        ...
        try:
            ...
            channel_resp = requests.post(dm_channel_url, json={"recipient_id": discord_user_id}, headers=headers, timeout=5)
            if channel_resp.status_code != 200:
                logger.error(f"Failed to open Discord DM channel for {discord_user_id}: {channel_resp.text}")
                return
            ...
            msg_resp = requests.post(msg_url, json={"content": message}, headers=headers, timeout=5)
            if msg_resp.status_code not in [200, 201]:
                logger.error(f"Failed to send Discord DM message to channel {channel_id}: {msg_resp.text}")
        except Exception as e:
            logger.error(f"Failed to send Discord DM: {e}")

    @staticmethod
    def notify_user(db: Session, user_id: int, event_type: str, title: str, message: str):
        ...
        dispatch_methods = [p.dispatch_method for p in prefs] if prefs else ["toast"]  # Default to browser toast
        ...
        if "discord_dm" in dispatch_methods:
            ...
            if discord_id and bot_token:
                ...
                logger.warning(f"Unable to send Discord DM: discord_user_id={discord_id is not None}, bot_token={bot_token is not None}")
    ...
    def notify_global(db: Session, event_type: str, title: str, message: str):
        ...
        for r in rules:
            try:
                full_message = f"📣 **System Update [{event_type}]**\n**{title}**\n{message}"
                if r.webhook_url:
                    NotificationService.send_discord_webhook(r.webhook_url, full_message)
                    ...
                    if bot_token:
                        ...
                        requests.post(url, json={"content": full_message}, headers=headers, timeout=5)
            except Exception as e:
    ...
    def check_and_notify_favorites(db: Session, entry):
        ...
        try:
            ...
            for fav in favorites:
                ...
                if fav.item_type == "performer" and entry.performers:
                    ...
                    # Also check studio tags or naming
                        ...
                        studio_meta = str(entry.entry_metadata.get("studio", "")).lower()
                        if val == studio_meta:
                            matched = True
                            matched_reason = f"studio '{fav.item_id}'"
            ...
            for user_id, reasons in user_matches.items():
                ...
                NotificationService.notify_user(db, user_id, "favorite_updated", title, message)
