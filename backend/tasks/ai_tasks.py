...
def auto_tag_video_task(self, library_entry_id: int):
    ...
    try:
        ...
        WebhookService.trigger("ai_tagging.completed", {"library_entry_id": entry.id, "new_tags": detected_tags})
    except ImportError:
        logger.error("AI packages (torch, transformers) not installed. Skipping AI tagging.")
    finally:
        db.close()