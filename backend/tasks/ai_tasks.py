...
def extract_frame_base64(file_path: str) -> str:
    ...
    try:
        ...
        cmd_duration = [
            ...
            "-of", "default=noprint_wrappers=1:nokey=1", abs_file_path
        ]
        # SECURITY: Add timeout to prevent process hanging (Denial of Service)
        duration_str = subprocess.check_output(cmd_duration, timeout=30).decode("utf-8").strip()  # nosec B603
        ...
        try:
            ...
            cmd_extract = [
                "ffmpeg", "-y", "-ss", str(midpoint), "-i", abs_file_path,
                "-vframes", "1", "-q:v", "2", frame_path
            ]
            subprocess.run(cmd_extract, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True, timeout=60)  # nosec B603
...
def extract_frame_at_timestamp_base64(file_path: str, timestamp: float) -> str:
    ...
    try:
        cmd_extract = [
            "ffmpeg", "-y", "-ss", str(timestamp), "-i", abs_file_path,
            "-vframes", "1", "-q:v", "2", frame_path
        ]
        subprocess.run(cmd_extract, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True, timeout=60)  # nosec B603
        ...
        logger.error(f"Timeout extracting frame at {timestamp} from {abs_file_path}: {e}")
        return ""
    except Exception as e:
        logger.error(f"Failed to extract frame at {timestamp} from {abs_file_path}: {e}")
        return ""
    finally:
        if os.path.exists(frame_path):
            os.remove(frame_path)
...
def get_scene_changes(file_path: str, threshold: float = 0.3) -> list[float]:
    ...
    try:
        ...
        output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, timeout=300).decode("utf-8")  # nosec B603
        ...
        try:
            ...
            duration = float(subprocess.check_output(cmd_duration, timeout=30).decode("utf-8").strip())  # nosec B603
...
def call_vision_model_text(base64_img: str, url: str, api_key: str, prompt: str) -> str:
    if url:  # Local Ollama
        try:
            payload = {"model": "llava", "prompt": prompt, "images": [base64_img], "stream": False}
            resp = requests.post(f"{url.rstrip('/')}/api/generate", json=payload, timeout=60)
            resp.raise_for_status()
...
def generate_video_chapters_task(self, library_entry_id: int, min_chapter_duration: float = 120.0):
    with get_db_session() as db:
        entry = db.query(LibraryEntry).filter(LibraryEntry.id == library_entry_id).first()
        if not entry or not os.path.exists(entry.file_path):
            return "Entry not found"
        ...
        logger.info(f"Generating {len(filtered_timestamps)} chapters for {entry.file_path}...")
        ...
        db.query(VideoChapter).filter(VideoChapter.library_entry_id == entry.id).delete()
        ...
        for i, ts in enumerate(filtered_timestamps):
            ...
            ai_title = call_vision_model_text(base64_frame, ollama_url.value if ollama_url else None, openai_key.value if openai_key else None, prompt)
            if ai_title: 
                chapter_title = ai_title.strip('"').strip("'")
            
            next_ts = filtered_timestamps[i+1] if i + 1 < len(filtered_timestamps) else (entry.duration or (ts + 300))
