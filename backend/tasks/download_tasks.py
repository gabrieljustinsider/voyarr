...
def real_download_task(self, task_id: int, prefs_dict: dict, metadata: dict):
    ...
    try:
        ...
        if active_cookie:
            vault_entry = db.query(Vault).filter_by(entity_type='session_cookie', entity_id=active_cookie.id, key='cookie_text').first()
            if vault_entry:
        ...
        if 'cookie_temp_path' in locals() and cookie_temp_path and os.path.exists(cookie_temp_path):
            try:
