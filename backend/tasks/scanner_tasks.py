    ...
    try:
        ...
        for root, _, files in os.walk(directory):
            for file in files:
                ...
                existing = db.query(LibraryEntry).filter(LibraryEntry.file_path == file_path).first()
                if existing:
                    continue
                ...
                if match:
                    ...
                    if 'performers' in data and data['performers']:
                        performers = [p.strip() for p in data['performers'].split(provider.separator)]
                    resolution = data.get('resolution')
                    
                    # Embed metadata into the physical file using Mutagen
                    MediaTagger.tag_file(file_path, {
                        "title": title,
                ...
                entry = LibraryEntry(
                    provider_id=provider.id, title=title, performers=performers,
                    file_path=file_path, file_size=os.path.getsize(file_path),
                    resolution=resolution, ohash=ohash, phash=phash
                )
                db.add(entry)
                processed += 1
