...
def transcode_video_task(self, transcode_job_id: int):
    ...
    library_entry = db.query(LibraryEntry).filter(LibraryEntry.id == job.library_entry_id).first()
    ...
    try:
        ...
        stream = ffmpeg.output(
            ...
            **{'crf': 28} # Constant Rate Factor (lower is better quality, 28 is a good default)
        )
        ...
        library_entry.entry_metadata['previous_file'] = {'path': old_file_path, 'size': old_file_size}
