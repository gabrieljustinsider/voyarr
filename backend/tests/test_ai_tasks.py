...
def test_call_openai_vision(mock_post):
    ...
    mock_response.json.return_value = {
        "choices": [{"message": {"content": "tagA, tagB, tagC"}}]
    }
    mock_response.raise_for_status.return_value = None
    mock_post.return_value = mock_response
...
def test_auto_tag_video_task_success(
    mock_webhook, mock_call_ollama, mock_extract, mock_exists, mock_get_db
):
    ...
    mock_webhook.trigger.assert_called_once_with(
        "ai_tagging.completed", {"library_entry_id": 42, "new_tags": ["tagA", "tagB"]}
    )
...
@patch("tasks.ai_tasks.extract_frame_base64")
@patch("tasks.ai_tasks.call_ollama_vision")
def test_auto_tag_video_task_retry(
    mock_call_ollama, mock_extract, mock_exists, mock_get_db
):
    import requests
...
@patch("tasks.ai_tasks.call_ollama_vision")
@patch("tasks.ai_tasks.WebhookService")
def test_auto_tag_video_task_fallback(
    mock_webhook, mock_call_ollama, mock_extract, mock_exists, mock_get_db
):
    import requests
