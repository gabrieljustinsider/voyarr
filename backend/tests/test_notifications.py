...
def test_update_preferences():
    ...
    task_completed_toast = next(
        p
        for p in res_data
        if p["event_type"] == "task_completed" and p["dispatch_method"] == "toast"
    )
    favorite_updated_dm = next(
        p
        for p in res_data
        if p["event_type"] == "favorite_updated"
        and p["dispatch_method"] == "discord_dm"
    )
