...
def test_live_streams_crud_and_record():
    ...
    response = client.post(
        "/live-streams",
        json={"name": "CB Stream", "url": "https://chaturbate.com/some_girl"},
    )
    assert response.status_code == 201
    ...
    with patch("celery.app.task.Task.delay") as mock_delay:
        ...
        assert (
            ...
            == "Background recording task spawned successfully."
        )
    ...
    with patch("celery.app.control.Control.revoke") as mock_revoke:
        ...
        assert (
            response.json()["message"]
            == "Recording stop signal dispatched successfully."
        )
        mock_revoke.assert_called_once_with(
            "celery-task-id-123", terminate=True, signal="SIGKILL"
        )
...
def test_live_stream_auth_and_stream():
    ...
    with (
        patch("routers.live_streams.shutil.which") as mock_which,
        patch("routers.live_streams.subprocess.run") as mock_run,
    ):
        ...
        assert (
            ...
            == "https://hls-edge-server.example.com/playlist.m3u8"
        )
...
def test_stash_stats_sync(mock_requests_post):
    ...
    print(f"DEBUG SYNC - SessionLocal in test: {SessionLocal}")
    print(f"DEBUG SYNC - db_utils.SessionLocal in test: {db_utils.SessionLocal}")
    # Seed local stats (plays is higher locally, climax is higher in stash)
    db = SessionLocal()
    from models import Provider, LibraryEntry, UserVideoStats

    # Ensure provider and library entry exist
    if not db.query(Provider).filter_by(id=1).first():
        db.add(Provider(id=1, name="Test Provider", base_url="http://test"))
        db.commit()

    if not db.query(LibraryEntry).filter_by(id=1).first():
        db.add(
            LibraryEntry(
                id=1, provider_id=1, title="Test Video", file_path="/path/to/video.mp4"
            )
        )
        db.commit()
