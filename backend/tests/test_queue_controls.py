...
def setup_db_and_dependencies():
    ...
    pref = DownloadPreference(
        provider_id=1, preferred_resolution="1080p", append_metadata=False
    )
    db.add(pref)
    ...
    media_entry = MediaEntry(
        id=1, provider_id=1, title="Test Download Video", ohash="123456789abcdef1"
    )
    db.add(media_entry)
...
def test_download_pause_resume_cancel(mock_revoke):
    ...
    mock_revoke.assert_called_once_with(
        "new-celery-456", terminate=True, signal="SIGTERM"
    )
...
def test_transcode_pause_resume_cancel(mock_revoke, mock_kill):
    ...
    mock_revoke.assert_called_once_with(
        "celery-t-123", terminate=True, signal="SIGKILL"
    )
...
def test_transcode_priority():
    db = SessionLocal()
    tq = TranscodingQueue(
        id=1, library_entry_id=1, target_codec="h265", status="pending", priority=1
    )
    db.add(tq)
    db.commit()
    db.close()
...
def test_live_stream_pause_resume_stop(mock_revoke, mock_kill):
    ...
    ls = LiveStream(
        id=1,
        name="Chaturbate Stream",
        url="http://cb.com/model",
        status="recording",
        pid=5678,
        current_task_id="celery-l-123",
    )
    db.add(ls)
    db.commit()
    db.close()
    ...
    mock_revoke.assert_called_once_with(
        "celery-l-123", terminate=True, signal="SIGKILL"
    )
